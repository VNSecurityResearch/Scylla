...
auto IATReferenceScan::numberOfFoundUniqueDirectImports () -> int
{
  auto apiPointers = std::set<DWORD_PTR>{};
  // std::for_each(std::begin(iatDirectImportList), std::end(iatDirectImportList),
  //               [&apiPointers](typename decltype(iatDirectImportList)::const_reference iat)
  //               {
  //                 apiPointers.insert(iat.targetAddressInIat);
  //               });
...
auto IATReferenceScan::patchDirectImportsMemory (bool junkByteAfterInstruction) -> void
{
  JunkByteAfterInstruction = junkByteAfterInstruction;
  for (auto&& iat : iatDirectImportList) {
    patchDirectImportInMemory(&iat);
  }
...
void IATReferenceScan::scanMemoryPage( PVOID BaseAddress, SIZE_T RegionSize )
{
  ...
  if (ProcessAccessHelp::readMemoryFromProcess((DWORD_PTR)BaseAddress, RegionSize, (LPVOID)dataBuffer))
  {
    while (1)
    {
      ...
      res = distorm_decompose(&ProcessAccessHelp::decomposerCi, ProcessAccessHelp::decomposerResult,
  ...
  if (META_GET_FC(instruction->meta) == FC_CALL || META_GET_FC(instruction->meta) == FC_UNC_BRANCH)
  {
    if (instruction->size >= 5)
    {
      ...
      if (instruction->flags & FLAG_RIP_RELATIVE)
      {
        ...
        Scylla::debugLog.log(PRINTF_DWORD_PTR_FULL L" " PRINTF_DWORD_PTR_FULL L" %S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL,
                             (DWORD_PTR)instruction->addr, ImageBase, inst.mnemonic.p,
                             inst.operands.p, instruction->ops[0].type, instruction->size, INSTRUCTION_GET_RIP_TARGET(instruction));
#endif
      ...
      if (instruction->ops[0].type == O_DISP)
      {
        ...
        Scylla::debugLog.log(PRINTF_DWORD_PTR_FULL L" " PRINTF_DWORD_PTR_FULL L" %S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL,
                             (DWORD_PTR)instruction->addr, ImageBase, inst.mnemonic.p,
                             inst.operands.p, instruction->ops[0].type, instruction->size, instruction->disp);
#endif
